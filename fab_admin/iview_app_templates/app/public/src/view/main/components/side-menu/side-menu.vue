  ...
  <div class="side-menu-wrapper">
    ...
    <div class="menu-collapsed" v-show="collapsed" :list="menuList">
      <template v-for="item in menuList">
        ...
        <Tooltip v-else-if="item.children && item.children.length==1" :content="item.children[0].meta.title" placement="right" :key="`drop-menu-${item.children[0].name}`">
        ...
        <a v-else @click="handleSelect(item.name)" class="drop-menu-a" :style="{textAlign: 'center'}"><Icon :size="rootIconSize" :color="textColor" :type="item.meta.icon"/></a>
      </template>
    </div>
