...
def import_application(app_package, appbuilder):
    ...
    try:
        import importlib
        _app = importlib.import_module(app_package)
#         _app = __import__(app_package)
    except Exception as e:
        click.echo(click.style('WARNING: Was unable to import {0} Error: {1}'.format(app_package, e), fg='red'))
        return
    if hasattr(_app, 'appbuilder'):
        return getattr(_app, appbuilder)
    else:
        click.echo(click.style('WARNING: There is no appbuilder var on your package, you can use appbuilder parameter to config', fg='red'))
        return
...
def cli_app():
    """
        A fabmanager CLI extension, to bring some useful command for fab_admin app clone
    """
    pass
...
def run(app, appbuilder, host, port, debug):
    ...
    """
    _appbuilder = import_application(app, appbuilder)
    _appbuilder.get_app.run(host=host, port=port, debug=debug, threaded=True)
...
def cli():
    import importlib
    try:
        importlib.import_module('fab_admin.commands')
        _appbuilder = import_application('app', 'appbuilder')
        _app = __import__('app.commands')
    except Exception as e:
        click.echo(click.style('There is no extend commands import Error: {0}'.format(e), fg='red'))
    cli_app()
